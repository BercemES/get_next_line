...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/02 14:31:59 by bekinci-          #+#    #+#             */
/*   Updated: 2024/12/02 14:31:59 by bekinci-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	stack[fd] = read_txt(fd, stack[fd]);
	if (!stack[fd])
		return (NULL);
	line = first_line(stack[fd]);
	stack[fd] = next_line(stack[fd]);
	return (line);
}
